...
public abstract unsafe partial class PacketDecoder
{
    ...
    private TextNode? DecodePacket(PacketID id, byte* payload) => id switch
    {
        ...
        PacketID.RetainerInfo when (RetainerInfo*)payload is var p => new($"#{p->SequenceId}.{p->Index}: {p->RetainerId:X} '{MemoryHelper.ReadSeStringNullTerminated((nint)p->Name)}', {p->NumItemsInInventory}/{p->NumItemsOnMarket} items, avail={p->Available}, u={p->Unk2A} {p->Unk2C}"),
        PacketID.RetainerMarketPriceSummary when (RetainerMarketPriceSummary*)payload is var p => new($"#{p->SequenceId}, {p->NumItemPackets} items"),
        PacketID.RetainerMarketPriceInfo when (RetainerMarketPriceInfo*)payload is var p => new($"#{p->SequenceId}, at {p->InventoryType}.{p->Slot}, unk={p->Unk10} {p->Unk18}"),
