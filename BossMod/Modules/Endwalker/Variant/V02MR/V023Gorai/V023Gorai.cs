...
{
    private List<Actor> _castersPurgationFirst = new();
    private List<Actor> _castersPurgationNext = new();
    ...
    {
        if (_castersPurgationFirst.Count > 0)
            return _castersPurgationFirst.Select(c => new AOEInstance(_shapePurgationFirst, c.Position, c.CastInfo!.Rotation, c.CastInfo!.NPCFinishAt));
        else
            return _castersPurgationNext.Select(c => new AOEInstance(_shapePurgationNext, c.Position, c.CastInfo!.Rotation, c.CastInfo!.NPCFinishAt));
    }
...
{
    private List<Actor> _castersSnapFirst = new();
    private List<Actor> _castersSnapSecond = new();
    private List<Actor> _castersSnapThird = new();
    ...
    public override IEnumerable<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        if (_castersSnapFirst.Count > 0)
            return _castersSnapFirst.Select(c => new AOEInstance(_shapeSnapFirst, c.Position, c.CastInfo!.Rotation, c.CastInfo!.NPCFinishAt));
        else if (_castersSnapSecond.Count > 0)
            return _castersSnapSecond.Select(c => new AOEInstance(_shapeSnapSecond, c.Position, c.CastInfo!.Rotation, c.CastInfo!.NPCFinishAt));
        else
            return _castersSnapThird.Select(c => new AOEInstance(_shapeSnapThird, c.Position, c.CastInfo!.Rotation, c.CastInfo!.NPCFinishAt));
    }
...
{
    private List<Actor> _castersPursuitFirst = new();
    private List<Actor> _castersPursuitRest = new();
