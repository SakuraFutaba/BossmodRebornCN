...
class YamaKagura(BossModule module) : Components.Knockback(module)
{
    ...
    public override bool DestinationUnsafe(int slot, Actor actor, WPos pos) => (Module.FindComponent<GhastlyGrasp>()?.ActiveAOEs(slot, actor).Any(z => z.Shape.Check(pos, z.Origin, z.Rotation)) ?? false) || !Module.Bounds.Contains(pos);
}
...
{
    private IReadOnlyList<Actor> _spirits = module.Enemies(OID.Spiritflame);
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, Contributors = "CombatReborn Team", PrimaryActorOID = (uint)OID.Boss, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 945, NameID = 12357)]
public class V022Moko(WorldState ws, Actor primary) : BossModule(ws, primary, new ArenaBoundsSquare(new(-700, 540), 20))
{
